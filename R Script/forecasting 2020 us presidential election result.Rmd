...
However, when merging groups to match the groups in the other data set, there is an unevitable loss of information. We are trying to complete this cleaning process with the reservation of important details the variables contain. Below, we mention some of the cleaning processes when we decide to merge some groups together, with detailed reasoning outlined in the Appendix - Notes on Data Cleaning Process-detailed:
...
2. Considering variable *race_ethnicity*, voters with different races may affect their voting decisions. We combine some small groups(sample) of variables into larger groups(population), and we end up with having 7 different levels.
...
(Table 1 shows the coefficients of variables and the intercept for voting Trump predicition.The estimated intercept for regression equation here is -0.1809. Since the variables have different level, the coefficients correspond to each level of the variable, for example, male increases the log odds while female does not have contributions in this cell. Positive coefficients show an increase in the log odds of Trump winning, and the larger the absolute value of the coefficient is, the more effect is has on the log odds.)
...
The building of this model gives society a timely and socially effect. As in the political field, voting for the two candidates can be a popular subject and news. The prediction of the model might affect public opinion. Predicting people's choices can be a timely and costly process, however, with the given data and a properly built model, the prediction will help people see how their choices affect the elections and notice the importance of personal choices. Even the data used in the model are not representative enough and are not collected by ourselves, it can let society have an idea of what the predicted result is. The social effect of such a model is great and relevant.
